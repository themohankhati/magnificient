    ...
    <header className="sticky top-0 z-40 w-full">
      ...
      <div className="bg-blue-900 overflow-hidden whitespace-nowrap">
        <p className="text-white py-1.5 text-sm animate-slide text-center">
          Celebrating 30 Years of Excellence – Serving since 1995 | Explore Nepal, Bhutan, Tibet & India | 📞 +977 1
          4523218, 4529264 | ✉️ info@magnificenttravels.com
        </p>
      </div>
      ...
      <div className="bg-white shadow-md w-full px-4 md:px-8 py-2 flex items-center justify-between mx-auto">
        ...
        <button onClick={toggleMenu} className="md:hidden focus:outline-none text-blue-900">
          {menuOpen ? <X size={28} /> : <Menu size={28} />}
        </button>

        {/* Desktop Nav */}
        <nav className="hidden md:flex items-center space-x-8 text-lg font-semibold">
          <Link to="/" className="relative text-gray-800 hover:text-blue-600 hover-underline">
            Home
          </Link>
          ...
          <div className="relative group">
            ...
            <ul className="absolute left-0 min-w-full bg-white border border-gray-200 rounded-lg shadow-lg opacity-0 invisible pointer-events-none group-hover:opacity-100 group-hover:visible group-hover:pointer-events-auto transition-all duration-200 z-50">
              <li>
                <Link to="/tour/nepal" className="block px-4 py-2 hover:bg-blue-100 text-gray-800">
                  Nepal
                </Link>
              </li>
              <li>
                <Link to="/tour/tibet" className="block px-4 py-2 hover:bg-blue-100 text-gray-800">
                  Tibet
                </Link>
              </li>
              <li>
                <Link to="/tour/bhutan" className="block px-4 py-2 hover:bg-blue-100 text-gray-800">
                  Bhutan
                </Link>
              </li>
              <li>
                <Link to="/tour/india" className="block px-4 py-2 hover:bg-blue-100 text-gray-800">
                  India
                </Link>
              </li>
            </ul>
          ...
          <Link to="/contactus" className="relative text-gray-800 hover:text-blue-600 hover-underline">
            Contact Us
          </Link>
        </nav>
        ...
        <div className="hidden md:flex items-center space-x-4">
          {loggedInUser ? (
            <>
              <span className="text-blue-800 font-semibold">Welcome, {loggedInUser?.first_name}</span>
              <button
            ...
            <>
              <Link
                to="/login"
                className="px-4 py-2 border border-blue-900 text-blue-900 rounded-lg font-medium hover:bg-blue-800 hover:text-white transition"
              >
                Login
              </Link>
              <Link
                to="/register"
                className="px-4 py-2 bg-blue-900 text-white rounded-lg font-medium hover:bg-blue-600 transition"
              >
                Register
              </Link>
            </>
      ...
      {menuOpen && (
        <div className="md:hidden bg-white shadow-md px-4 py-4 space-y-4 text-base font-medium">
          ...
          <div>
            ...
            {destOpen && (
              <div className="pl-4 pt-2 space-y-1">
                <Link to="/tour/nepal" onClick={toggleMenu} className="block text-gray-700">
                  Nepal
                </Link>
                <Link to="/tour/tibet" onClick={toggleMenu} className="block text-gray-700">
                  Tibet
                </Link>
                <Link to="/tour/bhutan" onClick={toggleMenu} className="block text-gray-700">
                  Bhutan
                </Link>
                <Link to="/tour/india" onClick={toggleMenu} className="block text-gray-700">
                  India
                </Link>
              </div>
            )}
          ...
          <Link to="/contactus" onClick={toggleMenu} className="block text-gray-800">
            Contact Us
          </Link>
          ...
          {loggedInUser ? (
            ...
            <div className="space-y-2 pt-2">
              <Link
                to="/login"
                onClick={toggleMenu}
                className="block w-full px-4 py-2 border border-blue-900 text-blue-900 rounded-lg text-center hover:bg-blue-800 hover:text-white transition"
              >
                Login
              </Link>
              <Link
                to="/register"
                onClick={toggleMenu}
                className="block w-full px-4 py-2 bg-blue-900 text-white rounded-lg text-center hover:bg-blue-600 transition"
              >
                Register
              </Link>
            </div>
